...
import { STATUS_OPTIONS, TaskHistoryGrid } from '../components/task-workflow/TaskHistoryGrid';
...
export function TaskHistoryPage() {
  ...
  const historyQ = useQuery({
    ...
    queryFn: async () => {
      ...
      return (await api.get<TaskHistoryPage>(`/workflow/history/global?${params.toString()}`))
        .data;
    },
  ...
  const bulkDeleteM = useMutation({
    ...
    onSuccess: () => {
      setSelectedIds([]);
      queryClient.invalidateQueries({ queryKey: ['workflow-history-global'] });
    },
  });
  ...
  function handleRestore(detail: RunDetail) {
    ...
    navigate(customTaskPath(projectId, detail.run.id), { state });
  }
        ...
        <>
          <div className="flex flex-wrap items-end gap-3">
            <label className="text-sm text-slate-600 dark:text-slate-300">
              ...
              <select
                ...
                value={projectFilter}
                onChange={(e) => updateParams({ projectId: e.target.value === 'all' ? null : e.target.value })}
              >
          ...
          {historyQ.isLoading ? (
            ...
            <TaskHistoryGrid
              ...
              onDeleted={() => queryClient.invalidateQueries({ queryKey: ['workflow-history-global'] })}
              emptyMessage={
                history?.total
                  ? 'No workflow runs match your filters.'
                  : 'No workflow runs yet. Start a task from a workspace to see history here.'
              }
